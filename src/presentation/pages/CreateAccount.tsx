...
const CreateAccount: React.FC<Props> = ({ navigation }: Props) => {
  ...
  function createAccountSuccess (): void {
    Alert.alert('Cadastor com sucesso.', 'Seja bem-vindo ' + name)
    clearState()
    setCreated(true)
  }

  function createAccountFailed (error): void {
    Alert.alert('Ocorreu um erro no seu cadastro', error.message)
  }
  async function onPressCreate (): Promise<void> {
    const user = {
      name,
      email,
      password
